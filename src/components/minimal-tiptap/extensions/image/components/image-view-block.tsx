...
import { type Editor, NodeViewWrapper, type NodeViewProps } from '@tiptap/react'
...
export const ImageViewBlock: React.FC<NodeViewProps> = ({ editor, node, selected, updateAttributes }) => {
  const { src: initialSrc, width: initialWidth, height: initialHeight, fileName } = node.attrs as {
  ...
  React.useEffect(() => {
    const handleImage = async () => {
      ...
      const { uploadFn } = (imageExtension?.options as { uploadFn?: (file: File, editor: Editor) => Promise<UploadReturnType> }) ?? {}
